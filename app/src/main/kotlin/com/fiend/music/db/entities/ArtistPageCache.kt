package com.fiend.music.db.entities
...
import com.fiend.innertube.pages.ArtistSection
...
fun serializeArtistPage(
    ...
    artist: com.fiend.innertube.models.ArtistItem? = null,
): String {
...
fun CachedArtistPage.toArtistPage(): com.fiend.innertube.pages.ArtistPage {
    return com.fiend.innertube.pages.ArtistPage(
        artist = artist?.let { cached ->
            com.fiend.innertube.models.ArtistItem(
                id = cached.id,
                title = cached.title,
        ...
        sections = sections.map { section ->
            ArtistSection(
                ...
                moreEndpoint = section.moreBrowseId?.let { browseId ->
                    com.fiend.innertube.models.BrowseEndpoint(
                        browseId = browseId,
...
private fun CachedItem.toYTItem(): YTItem {
    return when (this) {
        is CachedItem.Song -> SongItem(
            ...
            thumbnail = thumbnail,
            explicit = explicit,
            artists = artists.map { com.fiend.innertube.models.Artist(it.name, it.id) },
            album = album?.let { com.fiend.innertube.models.Album(it.name, it.id) },
            duration = duration,
            setVideoId = videoId,
        )
        is CachedItem.Album -> AlbumItem(
            ...
            thumbnail = thumbnail,
            explicit = explicit,
            artists = artists.map { com.fiend.innertube.models.Artist(it.name, it.id) }.ifEmpty { null },
            year = year,
        )
        is CachedItem.Playlist -> PlaylistItem(
            ...
            author = author?.let { com.fiend.innertube.models.Artist(it.name, it.id) },
        ...
        is CachedItem.Episode -> EpisodeItem(
            ...
            explicit = explicit,
            author = author?.let { com.fiend.innertube.models.Artist(it.name, it.id) },
            podcast = album?.let { com.fiend.innertube.models.Album(it.name, it.id) },
            duration = duration,
            endpoint = null,
        )
    }
